...
void CDX11VideoProcessor::SetDolbyVisionDynamicParams(float cw, float sg, float slope, float offset, float power, float MaxNits, float MinNits, float AvgNits)
{
	...
	{
		m_pDeviceContext->UpdateSubresource(m_pDoViDynamicConstants, 0, nullptr, &dynamic_cbuffer, 0, 0);
	}
	else {
		D3D11_BUFFER_DESC BufferDesc = {
			.ByteWidth = (sizeof(dynamic_cbuffer) + 15) & ~15,
			.Usage = D3D11_USAGE_DEFAULT,
			.BindFlags = D3D11_BIND_CONSTANT_BUFFER,
		};
		D3D11_SUBRESOURCE_DATA InitData = { &dynamic_cbuffer, 0, 0 };
...
BOOL CDX11VideoProcessor::InitMediaType(const CMediaType* pmt)
{
	...
	if (FmtParams.VP11Format != DXGI_FORMAT_UNKNOWN) {
		...
		if (SUCCEEDED(hr)) {
			...
			if (SourceIsHDR() && m_bHdrLocalToneMapping)
			{
